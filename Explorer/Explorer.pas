...
begin
 ...
 If (Node.Parent <> nil) and (not Node.HasChildren) then begin
  ...
  Chem := Chem + '\*.*' ;
         ...
         if FindFirst(Chem, faDirectory	, filename) = 0 then begin ;
             if ((filename.Attr and faDirectory) = faDirectory) and
                  (filename.Name <> '..') and (filename.Name <> '.') then begin
                  Pos := form1.TreeView1.Items.AddChild(Node, filename.Name) ;
                  Pos.ImageIndex := 1 ;
                  Pos.SelectedIndex := 1 ;
             end ;

            while FindNext(filename) = 0 do begin ;
             if ((filename.Attr and faDirectory) = faDirectory) and
                (filename.Name <> '..') and (filename.Name <> '.') then begin
                  Pos := form1.TreeView1.Items.AddChild(Node, filename.Name) ;
                  Pos.ImageIndex := 1 ;
  ...
  Liste : TStrings ;
    ...
    begin
      ...
      If (PosListe = -1) or (Extension = '.EXE') or (Extension = '.BMP') or
        ...
        Liste.Add(Extension) ;
          N := 0 ;
          H := ExtractAssociatedIcon(0, Name, N) ;
              Icone := TIcon.Create ;
              Icone.Handle := H ;
...
begin
  ...
  Pos := Node.getFirstChild ;
    while Pos <> nil do begin
     ...
     strpcopy(Name, Path(Node)+'\'+Caption);

       if (FindFirstFile(Name, p) <> INVALID_HANDLE_VALUE) then begin
 ...
 If (Node.Parent <> nil) then begin
  ...
  Chem := Chem + '\*.*' ;
         ...
         if FindFirst(Chem, faAnyFile, pathfile) = 0 then begin ;

            while FindNext(pathfile) = 0 do begin ;
             if ((pathfile.Attr and faDirectory) <> faDirectory) then
                with form1.ListView1.Items.Add do begin
                      Caption := pathfile.Name ;

                     strpcopy(Name, copy(chem, 0, length(chem)-3)+pathfile.Name);
                     ImageIndex := GetIcone(String(Name)) ;

                  FindFirstFile(Name, p);
                  FileTimeToSystemTime(p.ftLastWriteTime, Heures) ;
                  SubItems.Text := FormatFloat('#,###" Kb"', (Trunc(p.nFileSizeLow/1024)+1));
                  SubItems.Add(GetFileTypeString(ExtractFileExt(pathfile.Name))) ;
                  with Heures do
